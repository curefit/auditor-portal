-- Purpose: Count month-end membership base by business line, including all packs.
-- Output: month, business_line, user_base.
-- Membership-date fix: transferred/upgraded packs use membership-service created/start/end dates for month-end active windows.
with dates as (
...
membership_key,
business_line,
-- Transfers/upgrades should use membership-service dates so month-end membership counts follow the corrected pack window.
case
...
md.is_enterprise, md.amount_paid, md.pack_name, md.user_id, md.cancellation_date
